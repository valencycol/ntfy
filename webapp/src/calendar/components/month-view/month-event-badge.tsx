  ...
  return (
    <DraggableEvent event={event}>
      <EventDetailsDialog event={event}>
        <div role="button" tabIndex={0} data-event-badge="" className={eventBadgeClasses} onKeyDown={handleKeyDown}>
          ...
          {renderBadgeText && <span className="shrink-0">{event.allDay ? "All day" : format(new Date(event.startDate), "h:mm a")}</span>}
        </div>
      </EventDetailsDialog>
    </DraggableEvent>
  );
