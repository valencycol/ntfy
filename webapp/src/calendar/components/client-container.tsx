...
export function ClientContainer({ view }: IProps) {
  ...
  // The week and day views put "multi-day" events in a banner row above the
  // time grid, which is also where an all-day event belongs — otherwise a
  // one-day all-day event renders as a block covering all 24 hours. The month
  // and agenda views keep the plain span-based split.
  const banneredInTimeGrid = view === "week" || view === "day";

  const isSingleDay = (event: (typeof filteredEvents)[number]) => {
    if (banneredInTimeGrid && event.allDay) return false;
    return isSameDay(parseISO(event.startDate), parseISO(event.endDate));
  };

  const singleDayEvents = filteredEvents.filter(isSingleDay);
  const multiDayEvents = filteredEvents.filter(event => !isSingleDay(event));
